...
| 3.3a | A long segmented transfer arrives                             | The rate gate precedes the segmentation gate                                                                         | Every frame is charged and the reassembled unit is not charged again. Charging after segmentation claimed the frame left the whole path free          |
...
| 3.8  | Known category, unknown message type                          | Dispatch reports "no handler matched"                                                                                | Answered "unknown command"                                                                                                                            |
| 3.8a | Known message type, handler rejects the payload               | Dispatch distinguishes "matched but rejected" from "no match"                                                        | Answered "invalid payload", so the client is not sent looking for a command that does exist                                                           |
...
| 3.12 | A category destroyed while still registered                   | None                                                                                                                 | The list holds a dangling entry: the next frame for it, and the server's own destruction, both walk into freed memory. Unregister first. **Latent**   |
...
| 3.14 | A category acknowledges after being unregistered              | Unregistering clears the acknowledger it was given                                                                   | The same assertion, rather than a silent call into a server that no longer owns it                                                                    |
...
| 6.8 | The heartbeat falls due while the outbound queue is full         | Sending a heartbeat re-arms the heartbeat timer directly           | The heartbeat is dropped, and the next one is attempted an interval later. Re-arming only from the send notification would have stopped heartbeats for good, because a refused frame produces no notification |
...
| 10.9 | Segmentation destroyed before the protocol object it is attached to             | Detaching is the application's to do                                                    | The protocol object still holds the pointer its receive path dereferences. **Latent**: segmentation is built from that object's transport, so it dies first        |
