...
| 3.3a | A long segmented transfer arrives                             | The rate gate precedes the segmentation gate                                                                         | Every frame of the transfer is charged; the reassembled unit is not charged again. Charging after segmentation had claimed the frame would have left the whole multi-frame path free |
...
| 3.12 | A category destroyed while still registered                   | None                                                                                                                 | The registration list holds a dangling entry: the next frame for it dispatches into freed memory, and so does the server's own destruction, which walks the list. Unregister first. **Latent**: lifetime is the application's responsibility |
