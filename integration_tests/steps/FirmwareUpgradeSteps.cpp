#include "can-lite/categories/firmware_upgrade/FirmwareUpgradeDefinitions.hpp"
#include "can-lite/core/CanProtocolDefinitions.hpp"
#include "cucumber_cpp/Steps.hpp"
#include "steps/FirmwareUpgradeFixture.hpp"
#include "support/ApplicationFixture.hpp"
#include "gmock/gmock.h"
#include "gtest/gtest.h"

using namespace testing;
using namespace services;
using integration::ApplicationFixture;
using integration::FirmwareUpgradeFixture;

// Note: Cucumber {int} maps to std::int32_t. FirmwareSize and CRC32 are uint32 on the wire;
// test values must therefore fit within INT32_MAX (0x7FFFFFFF).

GIVEN(R"(the firmware upgrade category is registered on both client and server)")
{
    context.Emplace<FirmwareUpgradeFixture>(context.Get<ApplicationFixture>());
}

WHEN(R"(the client sends a begin upgrade command with firmware size {int})", (std::int32_t firmwareSize))
{
    auto& fixture = context.Get<FirmwareUpgradeFixture>();
    EXPECT_CALL(fixture.serverObserver, OnBeginUpgrade(static_cast<uint32_t>(firmwareSize), _));
    fixture.client.SendBeginUpgrade(fixture.application.config.nodeId, static_cast<uint32_t>(firmwareSize));
}

THEN(R"(the firmware upgrade server observer shall have received a begin upgrade with size {int})", (std::int32_t))
{
    SUCCEED();
}

WHEN(R"(the client sends data block {int})", (std::int32_t blockIndex))
{
    auto& fixture = context.Get<FirmwareUpgradeFixture>();
    EXPECT_CALL(fixture.serverObserver, OnDataBlock(static_cast<uint16_t>(blockIndex), _, _));
    hal::Can::Message blockData;
    blockData.push_back(0xAA);
    fixture.client.SendDataBlock(fixture.application.config.nodeId, static_cast<uint16_t>(blockIndex), blockData);
}

WHEN(R"(the client sends an abort command)")
{
    auto& fixture = context.Get<FirmwareUpgradeFixture>();
    EXPECT_CALL(fixture.serverObserver, OnAbort(_))
        .WillOnce(Invoke([](const infra::Function<void()>& onDone)
            {
                onDone();
            }));
    fixture.client.SendAbort(fixture.application.config.nodeId);
}

WHEN(R"(the client sends a verify command with CRC32 {int})", (std::int32_t crc32))
{
    auto& fixture = context.Get<FirmwareUpgradeFixture>();
    EXPECT_CALL(fixture.serverObserver, OnVerify(static_cast<uint32_t>(crc32), _));
    fixture.client.SendVerify(fixture.application.config.nodeId, static_cast<uint32_t>(crc32));
}

WHEN(R"(the client sends a query progress command)")
{
    auto& fixture = context.Get<FirmwareUpgradeFixture>();
    EXPECT_CALL(fixture.serverObserver, OnQueryProgress(_));
    fixture.client.SendQueryProgress(fixture.application.config.nodeId);
}

WHEN(R"({int} seconds elapse expecting a session timeout)", (std::int32_t seconds))
{
    auto& fixture = context.Get<FirmwareUpgradeFixture>();
    EXPECT_CALL(fixture.serverObserver, OnSessionTimeout());
    fixture.application.ForwardTime(std::chrono::seconds(seconds));
}

WHEN(R"({int} seconds elapse without a session timeout)", (std::int32_t seconds))
{
    auto& fixture = context.Get<ApplicationFixture>();
    fixture.ForwardTime(std::chrono::seconds(seconds));
}

THEN(R"(the firmware upgrade server observer shall have received a query progress notification)")
{
    SUCCEED();
}
